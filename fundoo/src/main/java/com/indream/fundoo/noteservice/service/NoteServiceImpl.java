...
public class NoteServiceImpl implements NoteService {
	 ...
	 */
	@Override
	public List<NoteEntity> getPinnedNotes(Token token) {
		String userId = this.getUserId(token);
		Iterable<NoteEntity> x = noteElasticRepository.findAll();
		Iterator<NoteEntity> value = x.iterator();
		List<NoteEntity> notes = new ArrayList<>();
		while (value.hasNext()) {
			NoteEntity noteEntity = (NoteEntity) value.next();
